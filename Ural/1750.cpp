...
int dblcmp(double x) {//为处理误差，所用的比较函数
    if (abs(x) < eps)
        return 0;
    return x>0?1:-1;
}
...
double area2(const Point &l, const Point &r, const Point &p) {  //求三点构成的有向三角形面积的二倍，
    return cross(r-l,p-l);                                      //前两个点为直线之端点
}

double getAngle(const Vector &u, const Vector &v) { //求两向量间夹角
    return acos(dot(u,v)/length(u)/length(v));
}

double getAngle(const Vector &v) { // 求向量与x轴所成夹角 值域(-pi,pi)
...
double p2segment(const Point &l, const Point &r, const Point &p) { //求点到线段的距离
    ...
    if (dblcmp(len) == 0) return length(p-l);
    Vector v1 = r-l, v2 = p-l, v3 = p-r;
    if (dblcmp(dot(v1,v2)) < 0) return length(v2);
    if (dblcmp(dot(v1,v3)) > 0) return length(v3);
...
typedef struct Segment {
    Point l, r;
    Segment() {}
    Segment(const Point &l, const Point &r): l(l), r(r) {}
} Line;//用两点也能确定一条直线
...
double getAngle(const Segment &s, const Segment &t) {
    double d = getAngle(s.r-s.l,t.r-t.l);
    if (d > PI/2)
        return PI-d;
    return d;
}
...
double intersectArea(const Circle &a, const Circle &b) {
    double d = dist(a.c,b.c);
    if (d >= a.r+b.r)
        return 0;
    if (d <= fabs(a.r-b.r))
...
bool aCmp(const Point &a, const Point &b) {//Andrew算法所需的水平序
    if (a.x != b.x)
        return a.x<b.x;
    return a.y<b.y;
}
...
struct HalfPlane: public Line {
    double angle;
    HalfPlane() { }
    HalfPlane(const Point &l, const Point &r): Line(l,r){
        angle = getAngle(r-l);
    }
    bool operator<(const HalfPlane &t) const {
        return angle < t.angle;
    }
};

bool onLeft(const HalfPlane &pl, const Point &p) {
    return dblcmp(area2(pl.l, pl.r, p)) > 0;
}

int halfPlaneIntersection(HalfPlane *pl, int n, Point *poly) { //半平面交，poly存储多边形，返回值为其顶点个数，0代表着无解
    sort(pl,pl+n);
    int first, last;
    Point *p     = new Point[n];
    HalfPlane *q = new HalfPlane[n];
    q[first=last=0] = pl[0];

    for (int i = 1; i < n; ++i) {
        while (first < last && !onLeft(pl[i], p[last-1]))
            --last;
        while (first < last && !onLeft(pl[i], p[first]))
            ++first;
        q[++last] = pl[i];
        if (0 == dblcmp(q[last].angle-q[last-1].angle)) {
            --last;
            if (onLeft(q[last], pl[i].l))
                q[last] = pl[i];
        }
        if (first < last)
...
int main() {
    ...
    while (cas--) {
        ...
        d[2][3] = d[3][2] = dist(A,B);
        d[2][4] = d[4][2] = dist(A,C);
        d[3][4] = d[4][3] = dist(B,C);

        if (!isIntersect(S,T,A,B) && !isIntersect(S,T,B,C)) {
            d[0][1] = d[1][0] = dist(S,T);
        }

        if (!isProperIntersect(S,A,B,C)) {
            d[0][2] = d[2][0] = dist(S,A);
        }

        if (!isProperIntersect(T,A,B,C)) {
            d[1][2] = d[2][1] = dist(T,A);
        }

        if (!isProperIntersect(S,C,A,B)) {
            d[0][4] = d[4][0] = dist(S,C);
        }

        if (!isProperIntersect(T,C,A,B)) {
            d[1][4] = d[4][1] = dist(T,C);
        }

        if (area2(B,A,S)*area2(B,C,S) >= 0) {
            d[0][3] = d[3][0] = dist(B,S);
        }

        if (area2(B,A,T)*area2(B,C,T) >= 0) {
            d[1][3] = d[3][1] = dist(B,T);
        }
        for (int k = 0; k < 5; ++k) {
            for (int i = 0; i < 5; ++i) {
                for (int j = 0; j < 5; ++j) {
