/**
 * Description: Given an arbitrary polygon and a point, check if the point is in the polygon.
 * Source:
 * Author: Ply_py
 */
#include <iostream>
...
Point getCentre(const Point *poly, int n) { //对多边形进行三角剖分求重心
    ...
    for (int i = 1; i < n-1; ++i) {
        ...
        ret = ret+pt*t;
        ret = ret+(poly[i]+poly[i+1])*(cross(poly[i],poly[i+1]));
    }
    ret = ret/(3*area);
    return ret;
}
...
int onConvex(const Point *ch, int n, const Point &p) { //判断点是否与凸多边形的关系，1在内部，0在边上，-1在外部
    int ret = 1;
    for (int i = 0; i < n; ++i) {
