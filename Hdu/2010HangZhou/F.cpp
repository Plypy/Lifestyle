...
Point poly[MAXN];
int n;
bool check(const Point &l, const Point &r, const Point &p) {
    return (-1 == dblcmp(dot(p-l, p-r)));
}
int main() {
    int T;
    scanf("%d", &T);
    for (int i = 0; i < T; ++i) {
        ...
        for (int i = 0; i < n; ++i)
            pt[i].read();
        Point centre = getCentre(pt, n);
        int m = andrew(pt, n, poly);
        poly[m] = poly[0];
        for (int i = 0; i < m; ++i) {
            Point A = poly[i], B = poly[i+1];
            Vector v = normal(B-A);
            Point C = centre + v;
            Point ip = getIntersection(A, B, C, centre);
            if (check(A,B,ip))
                ++ans;
        }
        printf("%d\n", ans);
