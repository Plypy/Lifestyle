...
bool newCheck(const Segment &seg, const Point &p) {
    if (dblcmp(cross(seg.r-seg.l, p-seg.l)) != 0)
        return false;
    if (dblcmp(dot(p-seg.r, p-seg.l)) >= 0)
        return false;
    return true;
}
int cut(Point *polygon, int n, const HalfPlane &plane, Point *newPoly) {
    int m = 0;
    Vector pv = plane.r-plane.l;
    for (int i = 0; i < n; ++i) {
        Point C = polygon[i], D = polygon[(i+1)%n];
        if (dblcmp(cross(pv, C-plane.l)) >= 0) {
            newPoly[m++] = C;
        }
        if (dblcmp(cross(pv, C-D)) != 0) {
            Line seg(C,D);
            Point ip = getIntersection(plane, seg);
            if (newCheck(seg, ip)) {
                newPoly[m++] = ip;
            }
        }
...
int main() {
    ...
    for (int i = 0; i < T; ++i) {
        ...
        for (int j = 0; j < n; ++j)
            vtx[j].read();
        int m = 4;
        core[0] = Point(INF,INF);
        core[1] = Point(-INF,INF);
        core[2] = Point(-INF,-INF);
        core[3] = Point(INF,-INF);
        for (int j = 0; j < n; ++j) {
            m = cut(core, m, HalfPlane(vtx[(j+1)%n],vtx[j]), tmp);
            for (int k = 0; k < m; ++k)
                core[k] = tmp[k];
        }
        double ans = polygonArea(core, m);
        printf("%.2lf\n", ans);
    }
    return 0;
}