...
int main() {
    ...
    for (int i = 0;  i < n; ++i) {
        Point a,b;
        a.read(), b.read();
        plane[i] = HalfPlane(a,b);
    }

    Point a(0,0), b(BOUND,0), c(BOUND, BOUND), d(0,BOUND);
    plane[n++] = HalfPlane(a,b);
    plane[n++] = HalfPlane(b,c);
    plane[n++] = HalfPlane(c,d);
    plane[n++] = HalfPlane(d,a);

    int m = halfPlaneIntersection(plane, n, poly);
    double ans = polygonArea(poly, m);
    printf("%.1lf\n", ans);
    return 0;
}