/**
 * Description: Check if a polygon is convex, if true, check if it is intersect with a given circle
 * Source: POJ 1584
 * Author: Ply_py
 */
#include <iostream>
...
double angle(const Vector &v) { // 求向量与x轴所成夹角 值域(-pi,pi)
...
bool check(Point *poly, int n) {
    bool r = false, l = false;
    for (int i = 0; i < n; ++i) {
        int t = dblcmp(area2(poly[i], poly[(i+1)%n], poly[(i+2)%n]));
        if (t > 0)
            l = true;
        if (t < 0)
            r = true;
...
bool contain(const Point *poly, int n, const Point &c, double r) {
    for (int i = 0; i < n; ++i) {
        double d = p2line(poly[i], poly[(i+1)%n], c);
        if (dblcmp(d-r) < 0)
            return false;
    }
    return true;
...
int main() {
    ...
    while (true) {
        ...
        if (onPolygon(poly, n, centre) <= 0 || !contain(poly,n,centre,r)) {
